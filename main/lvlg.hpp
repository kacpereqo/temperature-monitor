...
public:
	...
	void init()
	{
		...
		lv_display_set_theme(
		  disp,
		  lv_theme_default_init(disp, DARK_BG, rgb_fix(lv_color_make(255, 255, 255)), true, &lv_font_montserrat_14));
	...
	void main_screen()
	{
		...
		if (!is_initialized) {
			...
			for (size_t i = 0; i < 5; i++) {
				...
				lv_obj_set_style_text_color(dot_labels[i], rgb_fix({50, 50, 25}), 0);
